...
void LCDI2C_home(void){
	...
	Delayms(3000);  // this command takes a long time!
}
...
void LCDI2C_pulseEnable(uint8_t _data){
	...
	Delayms(50);		// commands need > 37us to settle
}
...
{	
	...
	{			
		...
		I2C_Send7bitAddress(I2C1, lcdi2c.Addr<<1, I2C_Direction_Transmitter);			
		Delayms(500);
	
		if(I2C1->SR1 & (1 << 7))		//Checking Txe Flag Bit in SR1 register 
		{
			I2C_GenerateSTOP(I2C1, ENABLE);
			return i;									//Return the matched address
		}
		else
		{				
			I2C_GenerateSTOP(I2C1, ENABLE);
